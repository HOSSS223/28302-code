...
volatile int BPM = 88; // int that holds raw Analog in 0. updated every 2mS
...
volatile boolean firstBeat = true; // used to seed rate array so we startup with a reasonable BPM
volatile boolean secondBeat = false; // used to seed rate array so we startup with a reasonable BPM
...
const float NORMAL_TEMP_MIN = 36.5;
const float NORMAL_TEMP_MAX = 37.5;
...
Serial.println("No GPS data. Check wiring and module.");
while (true);
}

vout = analogRead(sensorPin); // Reading the value from the sensor
vout = (vout * 500) / 1023;
...
// Print BPM, temperature, and pH value
Serial.print(BPM);
Serial.print(",");
Serial.print(tempc);
Serial.print(",");
...
// Check for a heart attack condition
if (BPM > 150 && tempc >= NORMAL_TEMP_MIN && tempc <= NORMAL_TEMP_MAX) {
Serial.println("Heart attack");
} else {
Serial.println("BPM higher than 150 and temperature is normal.");
}
...
arduinoSerialMonitorVisual('-', Signal); // goes to the function that makes Serial Monitor Visualizer
} else {
sendDataToSerial('S', Signal); // goes to the sendDataToSerial function
}
}
